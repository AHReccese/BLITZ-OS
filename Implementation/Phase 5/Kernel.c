  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        var i: int
          ...
          -- Set up the mutex' and condition
          threadManagerLock = new Mutex
          threadManagerLock.Init ()
          aThreadBecameFree = new Condition
          aThreadBecameFree.Init ()

          -- Set up the freeList for threads
          self.freeList = new List [Thread]

          -- Loop through the number of needed processes and set up a fresh
          -- thread with status UNUSED
          self.threadTable = new array of Thread {MAX_NUMBER_OF_PROCESSES of new Thread}
          for i = 0 to (MAX_NUMBER_OF_PROCESSES-1)
            --self.threadTable[i] = new Thread
            self.threadTable[i].Init ("Thread")
            self.threadTable[i].status = UNUSED
            self.freeList.AddToEnd (&self.threadTable[i])
          endFor
      ...
      method GetANewThread () returns ptr to Thread
        ...
        var
          ...
          -- Ensure thread manager only active once at any given time
          threadManagerLock.Lock ()

          -- Check and see if there's a thread free on the list
          -- If no thread free, wait on condition to indicate one is available
          while freeList.IsEmpty()
            aThreadBecameFree.Wait (& threadManagerLock)
          endWhile

          -- Thread is available! Grab it!
          th = freeList.Remove ()

          -- Indicate that this thread has been allocated
          th.status = JUST_CREATED

          -- Release lock so manager can continue on with other business
      ...
      method FreeThread (th: ptr to Thread)
        ...
        --
          ...
          -- Set thread status to indicate new state, add back to freeList.
          -- Adds to the end of the list for FIFO ordering of thread usage.
          -- (Simply because it pleases me.)
          th.status = UNUSED
          self.freeList.AddToEnd (th)

          -- Signal anyone waiting for a free thread that one is available
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        var i: int
          ...
          -- PID's start at 1, thusly it is "next"
          nextPid = 1

          -- Initialize our locks, conditions, and such
          ...
          -- Loop through the number of needed processes and set up a fresh
          -- ProcessControlBlock with status FREE
          self.processTable = new array of ProcessControlBlock {MAX_NUMBER_OF_PROCESSES of new ProcessControlBlock}
          for i = 0 to (MAX_NUMBER_OF_PROCESSES-1)
            self.processTable[i].Init ()
            self.processTable[i].status = FREE
            self.freeList.AddToEnd (&self.processTable[i])
          endFor
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        var
          ...
          -- Start by ensuring we only do one thing at a time
          self.processManagerLock.Lock ()

          -- Now we can see if there's a free PCB on the list, otherwise keep
          -- trying until there is.
          while looping == true
            if !(self.freeList.IsEmpty ())
              -- Available! Go!
              pcb = self.freeList.Remove ()

              -- Update it's status!
              pcb.status = ACTIVE

              -- Set it's PID, and ensure we increment the counter
              pcb.pid = nextPid
              nextPid = nextPid+1

              looping = false
            else
              -- Go to sleep and wait to try again...
              aProcessBecameFree.Wait (& processManagerLock)
            endIf
          endWhile

          -- We can unlock now, we're done.
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        --
          ...
          -- As usual, we must remain locked!
          processManagerLock.Lock ()

          -- Reset this processManagerBlock back to it's waiting state
          p.status = FREE
          self.freeList.AddToEnd (p)

          -- Signal to anyone waiting that they can have a go, and unlock
          -- the monitor for further use.
...
-----------------------------  InitFirstProcess  --------------------------
  ...
  function InitFirstProcess ()
    --
    -- This routine is called to get a user thread created and fork it to
    -- it's init function.
    --
    var
      th: ptr to Thread

      -- We need a thread, since this one returns.
...
-----------------------------  StartUserProcess  --------------------------
  ...
  function StartUserProcess (p: int)
    --
    -- This routine is called to spin up a new thread for a user process
    -- and get it ready to run. (This is the actula process thread.)
    --
    var
      pcb: ptr to ProcessControlBlock
      file: ptr to OpenFile
      initPC, initUserStackTop: int
      initSystemStackTop: ptr to int

      -- Allocate a new PCB
      pcb = processManager.GetANewProcess ()

      -- Connect the thread and the process.
      pcb.myThread = currentThread
      currentThread.myProcess = pcb

      -- Open the executable file.
      --   NOTE: Hardcoding the filename for this project, which was
      --         deemed acceptable until later.
      --   If there are problems with Open() it is unrecoverable error
      file = fileManager.Open ("TestProgram1")
      if(file == null)
        FatalError ("Could not open TestProgram1, unrecoverable.")
      endIf

      -- Create the logical address space and read executable into it. (OpenFile.LoadExecutable())
      --   if this fails it is unrecoverable
      --   LoadExecutable returns the entry point. Call it initPC
      initPC = file.LoadExecutable (&pcb.addrSpace)
      if(initPC == -1)
        FatalError ("Could not load executable unrecoverable.")
      endIf

      -- (Don't forget to close the executable file to prevent blocking the resource)
      fileManager.Close (file)

      -- Compute initial values for user and system stacks
      initUserStackTop = pcb.addrSpace.numberOfPages * PAGE_SIZE
      initSystemStackTop = &currentThread.systemStack[SYSTEM_STACK_SIZE-1]


      -- Turn this into a user-level thread...
      -- Disable interruputs, initialize page table registers, and let this
      -- thread know that it's a user thread.
      currentInterruptStatus = DISABLED
      pcb.addrSpace.SetToThisPageTable ()
      currentThread.isUserThread = true

      -- Execute the remaining register/PSW/interrupt changes atomically
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
        -- This method makes note of which frames have been allocated.

        var
          looping: bool = true
          i, frameAddr, frameIndex: int

          -- We must first ensure that we're doing one thing at a time...
          self.frameManagerLock.Lock ()

          -- Wait on a newFramesAvalaible if necessary
          while looping == true
            if (numFramesNeeded <= self.numberFreeFrames)
              -- We have enough frames, go for it!
              for i = 0 to numFramesNeeded-1
                -- Find a free frame
                frameIndex = self.framesInUse.FindZeroAndSet ()
                if frameIndex == -1
                  FatalError ("ABORT: No Free Frame Found. (We were told there was one!)")
                endIf

                -- Figure out the address of the free frame
                frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (frameIndex * PAGE_SIZE)

                -- Store the address of the frame which has been allocated
                aPageTable.SetFrameAddr (i, frameAddr)
              endFor

              -- Decrease our overall number of free frames
              self.numberFreeFrames = self.numberFreeFrames - numFramesNeeded

              -- Update the page table with the number of frames allocated
              aPageTable.numberOfPages = aPageTable.numberOfPages + numFramesNeeded

              -- Indicate that we can break the wait-loop and move on!
              looping = false
            else
              -- go to sleep until a new frame is signaled.
              self.newFramesAvailable.Wait (& self.frameManagerLock)
            endIf
          endWhile

          -- Release the manager to do further useful work.
          self.frameManagerLock.Unlock ()
        endMethod

      ----------  FrameManager . ReturnAllFrames  ----------

      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
        -- This method "gives back" all frames associated with this page table
        var i, tFrameAddr, tFrameOffset: int

          -- We must first ensure that we're doing one thing at a time...
          self.frameManagerLock.Lock ()

          for i = 0 to aPageTable.numberOfPages-1
            -- Extract the addr from this element of the page table, and then calculate
            -- it's offset by solving the address calculation for "f"
            tFrameAddr = aPageTable.ExtractFrameAddr (i)
            tFrameOffset = (tFrameAddr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME) / PAGE_SIZE

            -- Clear out the bit for this frame in the bitmap
            self.framesInUse.ClearBit (tFrameOffset)

            -- Now we can update the number of frames free
            self.numberFreeFrames = self.numberFreeFrames + 1
            aPageTable.numberOfPages = aPageTable.numberOfPages - 1
          endFor


          -- Release the manager to do further useful work, signalling anyone
          -- who is waiting for new frames.
          -- TODO: (maybe) Double-condition this sucker rather than simply broadcast,
          --       in order to address the starvation issue
          --    self.newFramesAvailable.Signal (& self.frameManagerLock)
          self.newFramesAvailable.Broadcast (& self.frameManagerLock)
          self.frameManagerLock.Unlock ()
        endMethod
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
    var
      ...
      -- Get the file name from the virtual address we were given
      fNameErr = currentThread.myProcess.addrSpace.GetStringFromVirtual(
                                                     &fName,
                                                     filename asInteger,
                                                     MAX_STRING_SIZE)
      if fNameErr < 0
        return -1 -- Return an error code. of sorts.
      endIf

      -- Set up the new addrSpace to use
      newAddrSpace.Init ()

      -- Open the executable file.
      file = fileManager.Open (&fName)
      if(file == null)
        return -1 -- Return an error code. of sorts.
      endIf
      ...
      -- (Don't forget to close the executable file to prevent blocking the resource)
      -- Do this right away so that even if the call fails the resource is closed.
      fileManager.Close (file)

      if(initPC == -1)
        return -1 -- Not sure that this is the correct failure response...
      endIf
      ...
      -- Make this new addrSpace we've built into the space to use.
      -- First we free up the old ones to prevent them from blocking indefinitely
      for i = 0 to currentThread.myProcess.addrSpace.numberOfPages-1
        currentThread.myProcess.addrSpace.ClearReferenced (i)
      endFor
