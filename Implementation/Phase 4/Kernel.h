header Kernel
  ...
  var
    ...
    -- Note:(for Task4)
    -- If you wanna use HOARE condition comment line above & uncomment line below
    -- Also go to line 31 of Main.c
    -- threadManager: HoareThreadManager
  ...
  ---------------  HoareMutex  ---------------
  -- NOTE:
  -- There is a differences in Unlock implementation of HOARE semantic & MESA semantic
  -- as you know in HOARE semantic the monitor lock is being PASSED among threads 
  -- so it is ok to unlock the lock that you don't own :) 
  ...
  class HoareCondition
    ...
    methods
      Init ()
      Wait (monitorLock: ptr to HoareMutex, nextSem: ptr to Semaphore, nextCount: int)
      Signal (monitorLock: ptr to HoareMutex, nextSem: ptr to Semaphore, nextCount: ptr to int)
  endClass
  ...
  --
  --  There is only one instance of this class, created at startup time.
  --  NOTE: See the documentation note about this in the implementation
  --        file!
  --
  class HoareThreadManager
    ...
    fields
      ...
      -- fields customized and needed for HOARE semantic.
