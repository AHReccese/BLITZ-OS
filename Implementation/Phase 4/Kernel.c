...
---------------------------  HoareMutex  -------------------------------
  ...
  behavior HoareMutex
    -- This class is designed for the Hoare monitor implementation
       ...
       method Unlock ()
           ...
           -- HOARE CONDITION 
           -- This is removed because locks are being Passed around between threads
           -- so it is ok to unlock the lock you dont own.
           -- if heldBy != currentThread
           --   FatalError ("Attempt to unlock a mutex by a thread not holding it")
           -- endIf
           -- END BLOCK
...
---------------------------  HoareCondition  -------------------------------
  ...
  behavior HoareCondition
      ...
      method Init ()
          self.cnt = 0
          self.sem = new Semaphore
          self.sem.Init (0)
        endMethod

      ----------  HoareCondition . Wait  ----------

      method Wait (monitorLock: ptr to HoareMutex, nextSem: ptr to Semaphore, nextCount: int)
      ...
      method Signal (monitorLock: ptr to HoareMutex, nextSem: ptr to Semaphore, nextCount: ptr to int)
        -- incepted from Harry Porter's suggested implementation
          if cnt > 0
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        var i: int
          ...
          -- Loop through the number of needed processes and new 
          -- threads with status UNUSED
      ...
      method GetANewThread () returns ptr to Thread
        ...
        var
          th: ptr to Thread
          doloop: bool = true -- needed because we are in MESA semantic.

          -- mutual exclusion
          threadManagerLock.Lock ()

          -- Check and see if there's a thread free on the list
          -- If there is then Grab it
          -- Else wait on condition :(
          while doloop
            if !(freeList.IsEmpty ())
              -- lets grab it(since mutual exclusion is satisfied by
              --              threadManagerLock we are sure no one else
              --              will grab it).
      ...
      method FreeThread (th: ptr to Thread)
        ...
        --
          ...
          -- mutual exclusion.
          threadManagerLock.Lock ()
          -- put thread back to thread list.
          th.status = UNUSED
...
-----------------------------  HoareThreadManager  ---------------------------------
  -- NOTE:
  -- I have used Harry Poeter's "IdeasOnHoareTask" to implement HOARE semantic.


  behavior HoareThreadManager

      ----------  HoareThreadManager . Init  ----------
      ...
      ----------  HoareThreadManager . GetANewThread  ----------
      ...
      method GetANewThread () returns ptr to Thread
        ...
        var
          ...
          -- it could be written without "while" syntax
          -- since it is using HOARE semantic 
          -- Check and see if there's a thread free on the list
          -- If there is then Grab it
          -- Else wait on condition :(
          while doloop 
            if !(freeList.IsEmpty ())
              ...
              aThreadBecameFree.Wait (& self.threadManagerLock, & self.nextSem, self.nextCount)
            endIf
          endWhile
      ...
      method FreeThread (th: ptr to Thread)
        ...
        --
          ...
          -- Signal for waiters :) 
          aThreadBecameFree.Signal (& self.threadManagerLock, & self.nextSem, & self.nextCount)
          
          -- conditionally Releasing the lock(remember we are in HOARE semantic)
          -- HOARE CONDITION 
          if self.nextCount > 0
            nextSem.Up ()
          else
  ...
  behavior ProcessManager
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        --
          ...
          -- put back process into free processes
          p.status = FREE
          self.freeList.AddToEnd (p)

          -- signal in order to feed waiters.
          ...
          while doloop 
            if (numFramesNeeded <= self.numberFreeFrames)
              ...
              for i = 0 to numFramesNeeded-1
                ...
                if frameIndex == -1
                  FatalError ("ABORT: No Free Frame Found.(Contradiction found among numberFreeFrames&reality).")
                endIf
                ...
                -- simple consideration that we getAllFrame we needed in start.
                -- I mean we call it at the first filling of pageTable.
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
        ...
        var i, tFrameAddr, tFrameOffset: int
          ...
          for i = 0 to aPageTable.numberOfPages-1
            -- get the addr from the page table
            -- and then extract it's offset.
