  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Lock ()
        ...
        -- Prevent a thread from getting the lock he already has
        if currentThread == owner
          FatalError ("Can't get a lock you already have.")
        endIf

        oldStat = SetInterruptsTo (DISABLED)

        if !owner
          -- If there's no owner -> transfer the ownership.
          owner = currentThread
        else
          -- This lock is already owned, so fall asleep :)
          waitList.AddToEnd (currentThread)
          currentThread.Sleep ()
        endIf

        -- Set interrupts back to what it was.
      ...
      method Unlock ()
        ...
        -- Prevent a thread from unlocking a lock illegally.
        if currentThread != owner
          FatalError ("You can't unlock the lock you don't own.")
        endIf

        oldIStat = SetInterruptsTo (DISABLED)

        -- get the next waiting thread
        nextThread = waitList.Remove ()
        if nextThread -- becomes false when nextThread is null.
          -- Transfer the lock's ownership to this thread :).
          owner = nextThread
          owner.status = READY
          readyList.AddToEnd (owner)
        else
          -- There's no waiting thread so owner is null(lock isn't acquired yet). 
          owner = null
        endIf

        -- Set interrupts back to what it was.
        oldIStat = SetInterruptsTo (oldIStat)
