...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    /* declare semaphore & needed mutex */
    mtLock: Mutex = new Mutex
  ...
  function ProducerConsumer ()
      ...
      /* initial semaphore & needed mutex */
      mtLock.Init ()
...
*/
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 5
        ...
        mtLock.Lock () -- handling multi producer multi consumer
...
/*
User defined function -> in order to increase readability.
    method TryForForks ()
      -- Check out whether can get forks or not.
      -- ...unimplemented...
      endMethod
*/
    method TryForForks (p: int)
    
      -- check whether Philosopher 'p' can start eating :)
      if status[p] == HUNGRY
         && status[(p+1) % 5] != EATING
         && status[(p+4) % 5] != EATING
...
*/
    ...
    method PickupForks (p: int)
      ...
      -- make sure that only one thing at
      -- a time is happening in the monitor.
      monActive.Lock ()
      ...
      while status[p] != EATING
        ...
        if status[p] != EATING
          -- forks are not available so wait.
          cond.Wait (& monActive)
        endIf
...
*/ 
    ...
    method PutDownForks (p: int)
      ...
      -- Put down forks and start thinking :)
      status[p] = THINKING
      self.PrintAllStatus ()

      -- let the others use forks :0
